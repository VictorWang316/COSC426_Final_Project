...
df = pd.read_csv("/home/zwang1/cosc426/Final/Dataset/new_news_title.txt", sep="\t", names=["textid", "text", "condition", "label"])
...
# Rename 'label' column to 'target' in all DataFrames
train_df = train_df.rename(columns={"label": "target"})
val_df = val_df.rename(columns={"label": "target"})
test_df = test_df.rename(columns={"label": "target"})

# Save each DataFrame to a file with the correct format and headers
train_df.to_csv("/home/zwang1/cosc426/Final/Dataset/New/news_title_training.txt", sep="\t", index=False)
val_df.to_csv("/home/zwang1/cosc426/Final/Dataset/New/news_title_validating.txt", sep="\t", index=False)
test_df.to_csv("/home/zwang1/cosc426/Final/Dataset/New/news_title_testing.txt", sep="\t", index=False)
